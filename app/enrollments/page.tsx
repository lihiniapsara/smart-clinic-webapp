...
function EnrollmentsContent() {
  ...
  const filtered = enrollments.filter((e) => {
    const matchesSearch =
      ...
      e.doctorSlotId.toLowerCase().includes(search.toLowerCase());
    const matchesProgram =
      programFilter === "all" || e.doctorSlotId === programFilter;
    ...
    <>
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <div className="relative w-full sm:w-64">
              ...
              <Input
                className="pl-9"
                placeholder="Search patient or doctor slot…"
                value={search}
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={programFilter} onValueChange={setProgramFilter}>
                <SelectTrigger className="w-48">
                  <SelectValue placeholder="All Doctor Slots" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">All Doctor Slots</SelectItem>
                  {programs.map((p) => (
        ...
        <div className="flex items-center gap-4 text-sm text-slate-500">
          <span>
            <strong className="text-slate-900">{filtered.length}</strong> appointment
            {filtered.length !== 1 ? "s" : ""}
          </span>
          {programFilter !== "all" && (
            <Badge
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-slate-50">
                ...
                <TableHead>Doctor Slot</TableHead>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Avatar className="h-7 w-7">
                          <AvatarImage
                            src={studentApi.getPictureUrl(e.patientId)}
                          />
                          <AvatarFallback className="bg-blue-100 text-blue-700 text-xs font-bold">
                            {initials(e.patient?.name ?? e.patientId)}
                          </AvatarFallback>
                        </Avatar>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{deleteTarget?.patient?.name ?? deleteTarget?.patientId}</strong>{" "}
              in doctor slot <strong>{deleteTarget?.doctorSlotId}</strong>? This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
